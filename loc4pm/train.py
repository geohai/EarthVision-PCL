...
def run(cfg_path: str, overrides=None):
    ...
    const_writer_path = os.path.join(cfg['project']['log_dir'], cfg['logging']['run_name'])
    writer = SummaryWriter(const_writer_path) if cfg['logging']['tensorboard'] else None

    # CSV logging setup
    save_epoch_csv = bool(cfg['logging'].get('save_epoch_csv', False))
    epoch_csv_path = cfg['logging'].get('epoch_csv')
    save_step_csv = bool(cfg['logging'].get('save_step_csv', False))
    step_csv_path = cfg['logging'].get('step_csv')
    ...
    for epoch in range(1, cfg['train']['epochs']+1):
        ...
        if val_loader is not None:
            ...
            # append per-epoch CSV row (before scheduler step)
            ...
            if val_loss < best_val:
                ...
                torch.save({'model': model.state_dict(), 'cfg': cfg, 'epoch': epoch},
                           os.path.join(cfg['project']['ckpt_dir'], 'best.pt'))
            if sched is not None:
                sched.step()
